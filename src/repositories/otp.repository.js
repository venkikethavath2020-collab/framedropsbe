...
export async function create({ id, email, codeHash, expiresMinutes }) {
  const { rows } = await query(
    `INSERT INTO otp_codes (id, email, code_hash, expires_at)
     VALUES ($1, $2, $3, now() + make_interval(mins => $4))
     RETURNING id, email, expires_at, attempt_count, used`,
    [id, email, codeHash, expiresMinutes]
  )
  return rows[0]
}

/**
 * Return the most recent unused, unexpired row for this email. The service
 * compares `code_hash` in constant time (string === on a fixed-length
 ...
 */
export async function findLatestActive(email) {
  const { rows } = await query(
