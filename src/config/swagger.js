...
const definition = {
  ...
  tags: [
    ...
    { name: 'Photos',              description: 'Per-album photo CRUD, R2 upload sign + finalize.' },
    ...
    schemas: {
      ...
      Photo: {
        ...
        properties: {
          ...
          cloudinaryId: { type: 'string', example: 'framedrops/<albumId>/<uuid>', description: 'LEGACY: only set on rows from before the R2 cutover. New rows have storageKey instead.' },
          storageKey:   { type: 'string', example: 'framedrops/<albumId>/<uuid>', description: 'R2 object key. Primary identifier for new rows.' },
      ...
      SignedUpload: {
        type: 'object',
        description: 'Cloudflare R2 presigned PUT credentials. The frontend PUTs the file directly to R2 with these params — backend never touches the bytes.',
        properties: {
          provider:   { type: 'string', example: 'r2' },
          method:     { type: 'string', example: 'PUT' },
          uploadUrl:  { type: 'string', format: 'uri', example: 'https://<bucket>.<account>.r2.cloudflarestorage.com/framedrops/8e6b…/9f3a…?X-Amz-Signature=…' },
          headers:    {
            type: 'object',
            description: 'Headers the browser MUST send on the PUT — values must match exactly or the signature is rejected.',
            properties: {
              'Content-Type':  { type: 'string', example: 'image/jpeg' },
              'Cache-Control': { type: 'string', example: 'public, max-age=2592000, immutable' },
            },
          },
          storageKey: { type: 'string', example: 'framedrops/8e6b…/9f3a…' },
          publicId:   { type: 'string', example: '8e6b…/9f3a…', description: 'Kept for shape parity with legacy callers; on R2 it equals storageKey.' },
          publicUrl:  { type: 'string', format: 'uri', example: 'https://cdn.framedrops.in/framedrops/8e6b…/9f3a…' },
        },
      },
      LockedAlbumsSummary: {
        type: 'object',
