 ...
 * POST   /albums/:albumId/photos          — upload (multipart, via multer + Cloudinary)
...
// Only the proxy-upload (multipart, server-side compress) path still
// needs Cloudinary helpers in the controller. Direct-upload sign /
// finalize / delete now select their own provider in the service layer.
import { uploadImage, deleteImage } from '../config/cloudinary.js'
import * as R from '../utils/response.js'

export async function signPhotoUpload(req, res) {
  // Body may carry { fileName, fileType, fileSize } — required for R2's
  // POST policy, ignored by Cloudinary. Optional so existing clients keep
  // working against the cloudinary path without any update.
