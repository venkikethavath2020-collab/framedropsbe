...
import { deleteImages as cloudinaryDeleteImages } from '../config/cloudinary.js'
...
// Max price a photographer can charge customers (Flow 2), in INR (NOT paise)
const MAX_CLIENT_PRICE_INR = parseInt(process.env.PRICE_TIER_5 || '349', 10)
...
function validateFolderPrice(paise) {
  ...
  if (paise / 100 > MAX_CLIENT_PRICE_INR) {
    return { ok: false, error: `Gallery price cannot exceed ₹${MAX_CLIENT_PRICE_INR}` }
  }
...
export async function createClient(userId, body) {
  ...
  if (avatar != null && (typeof avatar !== 'string' || avatar.length > 100_000)) {
    return { error: 'Avatar must be a string (max 100 KB)', status: 400 }
  }
...
export async function updateClient(id, userId, body = {}) {
  ...
  if (body.avatar !== undefined) {
    if (body.avatar != null && (typeof body.avatar !== 'string' || body.avatar.length > 100_000)) {
      return { error: 'Avatar must be a string (max 100 KB)', status: 400 }
    }
...
export async function deleteClient(id, userId) {
  ...
  // Collect storage refs up-front (outside tx) so we know what to clean
  ...
  // is fire-and-forget per provider — orphaned bytes get swept by the
  // album-expiry / orphan-reaper crons if either delete fails.
  //
  // Provider-aware: an album can mix cloudinary + r2 photos when the
  // breaker tripped mid-upload, so the worker partitions per row.
  ...
  for (const album of albums) {
    ...
    for (const r of refs) {
      ...
      else if (r.provider === 'cloudinary') cloudinaryIds.push(r.key)
    }
  }
  ...
  // Both providers in parallel — independent backends, no shared state.
  await Promise.allSettled([
    cloudinaryIds.length ? cloudinaryDeleteImages(cloudinaryIds) : null,
    r2Keys.length        ? r2DeleteObjects(r2Keys)               : null,
  ].filter(Boolean)).then(results => {
    for (const r of results) {
      if (r.status === 'rejected') {
        console.error('[Client] storage cleanup failed (DB state is consistent):', r.reason)
      }
    }
  })
...
export async function shareClient(id, userId) {
  ...
  // UNIQUE constraint rather than surfacing a 500.
